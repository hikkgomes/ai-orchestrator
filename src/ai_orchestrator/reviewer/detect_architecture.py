"""Repository architecture detection helpers for reviewer configuration."""
...
LIBRARY_CATEGORIES = {
    "framework": ["react", "next", "vue", "nuxt", "svelte", "angular", "django", "flask", "fastapi", "express", "nestjs", "koa", "rails", "laravel", "spring", "gin", "fiber", "flutter"],
    "database": ["prisma", "sequelize", "typeorm", "drizzle", "knex", "mongoose", "sqlalchemy", "sqlmodel", "psycopg", "pg", "mysql", "postgres", "sqlite", "redis", "mongodb", "supabase"],
    "testing": ["jest", "vitest", "testing-library", "playwright", "cypress", "pytest", "rspec", "minitest", "unittest"],
    "http": ["axios", "requests", "httpx", "aiohttp", "faraday", "rest-client", "got"],
    "state_management": ["redux", "zustand", "mobx", "pinia", "recoil", "xstate"],
    "styling": ["tailwindcss", "styled-components", "emotion", "sass", "less", "bootstrap", "chakra-ui"],
    "auth": ["next-auth", "authjs", "passport", "clerk", "auth0", "firebase-auth", "devise"],
    "payments": ["stripe", "paypal", "braintree", "square"],
    "validation": ["zod", "yup", "joi", "pydantic", "marshmallow", "class-validator"],
}
DIRECTORY_LABELS = {
...
def detect_naming(files: list[Path]) -> dict[str, str]:
    ...
    for path in files[:80]:
        ...
        if path.suffix == ".py":
            ...
            if path.suffix in {".jsx", ".tsx"}:
                for _ in re.finditer(r"(?:export\s+default\s+function|export\s+function|const)\s+([A-Z][A-Za-z0-9]*)", text):
...
def project_description(root: Path) -> str:
    ...
    if description:
        return description
    package_description = str(read_json(root / "package.json").get("description") or "").strip()
    if package_description:
        return package_description
    pyproject = read_toml(root / "pyproject.toml")
    return str((pyproject.get("project") or {}).get("description") or "").strip()
...
def detect_architecture(root: Path) -> dict:
    """Detect architectural signals for *root*."""
