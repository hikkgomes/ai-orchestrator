"""Repository stack and command detection for reviewer configuration."""
...
def detect_for_directory(directory: Path) -> dict:
    ...
    if exists(directory, "pyproject.toml", "requirements.txt", "setup.py", "setup.cfg", "Pipfile", "poetry.lock", "uv.lock"):
        ...
        if exists(directory, "mypy.ini", "pyrightconfig.json"):
            result["commands"]["typecheck"] = result["commands"]["typecheck"] or ("mypy ." if (directory / "mypy.ini").exists() else "pyright")
...
def discover_from_cargo(root: Path) -> list[Path]:
    ...
    return expand_patterns(root, [member for member in members if isinstance(member, str)])


def discover_from_lerna(root: Path) -> list[Path]:
    data = read_json(root / "lerna.json")
    packages = data.get("packages") or []
    return expand_patterns(root, [package for package in packages if isinstance(package, str)])


def discover_from_nx(root: Path) -> list[Path]:
    if not (root / "nx.json").exists():
        return []
    patterns = [candidate for candidate in ["apps/*", "libs/*", "packages/*"] if any(root.glob(candidate))]
    return expand_patterns(root, patterns)
...
def discover_workspaces(root: Path) -> list[tuple[str, Path]]:
    explicit: OrderedDict[str, Path] = OrderedDict()
    for path in (
...
def detect_commands(root: Path) -> dict:
    """Detect stack, common commands, and workspaces for *root*."""
